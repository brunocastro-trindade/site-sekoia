  ...
  if (cta.href) {
    return (
      <Button asChild {...buttonProps}>
        <a href={cta.href}>{buttonProps.children}</a>
      </Button>
    );
  }
  return (
    <Button onClick={cta.onClick} {...buttonProps}>
...
export function GlowyWavesHero({
  eyebrow = "Reactive canvas hero",
  eyebrowIcon = <Sparkles className="h-4 w-4 text-primary" aria-hidden="true" />,
  heading = (
    <>
  ...
  description = "Build living surfaces that respond to every interaction. Craft cinematic hero moments, responsive canvases, and luminous gradients without leaving your design system.",
  primaryCta = { label: "Launch Studio" },
  secondaryCta,
  pills = defaultPills,
  stats = defaultStats,
  className = "",
  ...
  return (
    ...
    >
      ...
      {backdrop && (
        <div
          className={`absolute inset-0 z-[1] ${showCanvas ? "mix-blend-screen" : ""}`}
          aria-hidden="true"
        >
          {backdrop}
        </div>
      ...
      <div
        className={
          isLeft
            ? "relative z-10 flex w-full flex-col items-start px-5 py-24 text-left md:px-8"
            : "relative z-10 mx-auto flex w-full max-w-6xl flex-col items-center px-6 py-24 text-center md:px-8 lg:px-12"
        }
      >
        <motion.div
        ...
        >
          ...
          <motion.h1
            variants={itemVariants}
            className="mb-6 text-5xl font-semibold tracking-tight text-foreground md:text-7xl lg:text-8xl"
          >
            {heading}
          </motion.h1>
          ...
          {description && (
            <motion.p
              variants={itemVariants}
              className={`mb-10 max-w-3xl text-lg text-foreground/70 md:text-2xl ${isLeft ? "" : "mx-auto"}`}
            >
              {description}
            </motion.p>
          )}
          ...
          {(primaryCta || secondaryCta) && (
            <motion.div
              variants={itemVariants}
              className={`mb-10 flex flex-col gap-4 sm:flex-row ${isLeft ? "items-start" : "items-center justify-center"}`}
            >
              {primaryCta && (
                <CtaButton
                  cta={primaryCta}
